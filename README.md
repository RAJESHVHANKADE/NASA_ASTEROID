...
This project analyzes NASA asteroid data using Python and machine learning techniques. It aims to classify asteroids as hazardous or non-hazardous based on various features. The dataset is preprocessed, exploratory data analysis (EDA) is conducted, and a predictive model is built using the XGBoost classifier. The model's performance is evaluated using accuracy score, classification report, and confusion matrix. The project provides insights into asteroid characteristics and their potential impact on Earth.
